    ...
    with tab_dotacion:
        ...
        with col_top1:
            ...
            if not df_historia.empty:
                ...
                fig_evol.update_traces(textposition="top center", textfont_size=11, marker=dict(size=7, color="#1e293b"),
                                       line=dict(color="#475569", width=2), hovertemplate="%{text} Colaboradores<extra></extra>")
        ...
        with col_top2:
            ...
            if not df_chart_cat.empty and 'CATEGORIA' in df_chart_cat.columns:
                ...
                fig_cat.update_traces(hovertemplate="Categoría: %{y}<br>Colaboradores: %{text}<extra></extra>", textposition='outside')
        ...
        with col_x1:
            ...
            if not df_chart_emp.empty:
                ...
                fig_emp.update_traces(hovertemplate="%{x}<br>Dotación: %{text}<extra></extra>")
        ...
        with col_x2:
            ...
            if not df_chart_loc.empty:
                ...
                fig_loc.update_traces(textinfo='value+percent', hovertemplate="%{label}<br>Dotación: %{value} (%{percent})<extra></extra>")
        ...
        with col_x3:
            ...
            if not df_chart_area.empty:
                ...
                fig_area.update_traces(textinfo='percent', hovertemplate="%{label}<br>Dotación: %{value} (%{percent})<extra></extra>")
        ...
        with col_x4:
            ...
            if not df_chart_ant.empty:
                ...
                fig_ant.update_traces(hovertemplate="Rango: %{x}<br>Colaboradores: %{text}<extra></extra>")
        ...
        with col_x5:
            ...
            if col_lider:
                ...
                if not df_chart_lid.empty:
                    ...
                    fig_lid.update_traces(hovertemplate="Líder: %{y}<br>Personas a cargo: %{x}<extra></extra>")
        ...
        if filtros_activos:
            st.markdown(f"<div style='background:#f1f5f9; padding:15px; border-radius:8px; border-left: 4px solid #2563eb;'>? Nómina Interactiva ({len(df_tabla_final)} filtrados): {' | '.join(filtros_activos)}</div><br>", unsafe_allow_html=True)
            ...
            if len(altas_mes) > 0 or len(bajas_mes) > 0:
                ...
                with tab_altas:
                    if len(altas_mes) > 0:
                        ...
                        fig_a.update_traces(hovertemplate="%{x}<br>Altas: %{text}<extra></extra>")
                ...
                with tab_bajas:
                    if len(bajas_mes) > 0:
                        ...
                        with col_b1:
                            ...
                            fig_b.update_traces(hovertemplate="%{x}<br>Bajas: %{text}<extra></extra>")
                        ...
                        with col_b2:
                            ...
                            fig_mot.update_traces(textinfo='value+percent', hovertemplate="%{label}<br>Cantidad: %{value} (%{percent})<extra></extra>")
        ...
        try:
            ...
            if 'FECHA_MOV_DT' in df_mov.columns:
                ...
                if not df_mov_periodo.empty:
                    ...
                    st.markdown("<p style='font-size: 13px; color: #64748b;'>?? Consejo: Haz clic en el gráfico de torta para auditar los resultados de las evaluaciones de potencial.</p>", unsafe_allow_html=True)
                    ...
                    if bajas_temp_mov > 0:
                        ...
                        st.markdown(f"<div style='background-color: #fffbeb; border-left: 4px solid #f59e0b; padding: 12px; border-radius: 4px; margin-bottom: 15px;'><p style='color: #b45309; font-weight: 600; font-size: 14px; margin: 0;'>?? Riesgo de Retención post-Movimiento: {bajas_temp_mov} de los {total_movs} colaboradores movidos/promovidos ({pct_fracaso:.1f}%) se dieron de baja antes de cumplir 12 meses en su nuevo rol.</p></div>", unsafe_allow_html=True)
                    ...
                    with col_m1:
                        ...
                        if 'TIPO_MOV' in df_mov_periodo.columns:
                            ...
                            fig_tipo_mov.update_traces(textinfo='value+percent', hovertemplate="%{label}<br>Cantidad: %{value} (%{percent})<extra></extra>")
                    ...
                    with col_m2:
                        ...
                        if not df_eval.empty and 'POTENCIAL' in df_eval.columns:
                            ...
                            fig_pot.update_traces(hovertemplate="Potencial: %{x}<br>Cantidad: %{text}<extra></extra>")
                    ...
                    with st.expander("Ver detalle histórico de movimientos y promociones"):
                        ...
                        if sel_click_tipo_mov:
                            ...
                            st.markdown(f"<div style='font-size:13px; color:#2563eb; margin-bottom:10px;'>Filtro activo: Mostrando solo {sel_click_tipo_mov}</div>", unsafe_allow_html=True)
    ...
    with tab_rotacion:
        ...
        if not df_historia.empty:
            ...
            with tab_rot_tot:
                ...
                fig_rt.update_traces(textposition="top center", textfont_size=11, texttemplate='%{text:.1f}%', marker=dict(size=7, color="#b91c1c"), line=dict(color="#ef4444", width=2), hovertemplate="%{y:.1f}% Rotación Total<extra></extra>")
            ...
            with tab_rot_vol:
                ...
                fig_rv.update_traces(textposition="top center", textfont_size=11, texttemplate='%{text:.1f}%', marker=dict(size=7, color="#c2410c"), line=dict(color="#f97316", width=2), hovertemplate="%{y:.1f}% Rotación Voluntaria<extra></extra>")
            ...
            with tab_rot_temp:
                ...
                fig_rtemp.update_traces(textposition="top center", textfont_size=11, texttemplate='%{text:.1f}%', marker=dict(size=7, color="#a16207"), line=dict(color="#eab308", width=2), hovertemplate="%{y:.1f}% Rotación Temprana<extra></extra>")
        ...
        with col_r1:
            ...
            if tot_bajas_rot > 0:
                ...
                fig_tipo.update_traces(textinfo='value+percent', hovertemplate="%{label}<br>Bajas: %{value} (%{percent})<extra></extra>")
        ...
        with col_r2:
            ...
            if tot_bajas_vol_rot > 0:
                ...
                fig_area_vol.update_traces(hovertemplate="Área: %{y}<br>Renuncias: %{text}<extra></extra>")
        ...
        if tot_bajas_rot > 0:
            ...
            if filtros_rot:
                st.markdown(f"<div style='background:#fef2f2; padding:15px; border-radius:8px; border-left: 4px solid #ef4444; margin-bottom:15px;'>Filtro activo ({len(df_show_rot)} resultados): {' | '.join(filtros_rot)}</div>", unsafe_allow_html=True)
            else:
                st.markdown("<p style='font-size: 13px; color: #64748b;'>?? Consejo: Haz clic en los gráficos superiores para filtrar esta tabla y auditar detalles específicos.</p>", unsafe_allow_html=True)
